                ...
                <tr class="border-b border-[#EFEDE6] hover:bg-[#FAF9F5]">
                    ...
                    <td class="px-5 py-4">
                        <div class="flex justify-end gap-4 text-xs font-medium">
                            <a href="{{ route('users.show', ['id' => $user->id]) }}" class="text-[#16213A] hover:text-[#A16207]">
                                Lihat
                            </a>
                            <a href="{{ route('users.edit', ['id' => $user->id]) }}" class="text-[#16213A] hover:text-[#A16207]">
                                Ubah
                            </a>
                            <form action="{{ route('users.destroy', ['id' => $user->id]) }}" method="POST" onsubmit="return confirm('Hapus data pengguna ini?')">
                                @csrf
                                @method('DELETE')
